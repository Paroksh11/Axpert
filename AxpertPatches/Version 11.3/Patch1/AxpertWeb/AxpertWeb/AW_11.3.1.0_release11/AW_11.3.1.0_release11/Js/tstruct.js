...
function customTstHtml() {
    ...
    ShowDimmer(true);
    let myModal = new BSModal("tstCustomHtml", "Custom HTML", "<iframe class='col-12 h-100' src='./tstructhtml.aspx'></iframe>", () => {
        //shown callback
    }, () => {
        //hide callback
    });
...
function ValidateExpression(fldObj, fldno) {
    ...
    if (vexpr != undefined && vexpr != "" && (flval && (flval != "" || flval != dateString))) {
        ...
        if (_fldName != "" && AxFormControlList.length > 0) {
            let _thisEle = AxFormControlList.filter(x => x.indexOf('setfieldcaption~' + _fldName + '^') > -1);
...
function AssignJQueryEvents(dcArray) {
    ...
    for (var i = 0; i < dcArray.length; i++) {
        ...
        if ($j(dvId).length > 0) {
            ...
            $j(dvId + " select:not(.fldFromSelect,.multiFldChklist,.multiFldChk)").change(function () {
...
function CallEvaluateOnAddPerf(dcNo, rowNo, fields, calldepField) {
    ...
    for (var i = 0; i < fields.length; i++) {
        ...
        if (oldValue != newValue) {
            if (CheckDependencyPerf(fldName, "AddRow") == true && callService == false && calldepField != "Delete" && calldepField != "AllRowClear") {
                callService = true;
            }
        }
        ...
        unSelList.forEach(function (ele) {
            ...
            if (_isValueSelected) {
                if (_selValues.indexOf(_chkSep) > -1)
                    _selValues = _selValues.replace(_thisVal + _chkSep, "").replace(_chkSep + _thisVal, "");
                else
                ...
                if (theModeDesign == "true") {
                    ...
                    if (thisDcno == "" || thisDcno == _grflds[1]) {
                        ...
                        for (var i = 0; i <= _thIndex; i++) {
                            ...
                            $("#" + _thisGrd + " thead th:eq(" + i + ")").css({ "position": "sticky", "background-color": "rgb(251 218 141)", "left": offsetth + "px" });
...
function publishAPIInputJSON(thisFldid) {
    try {
        if (thisFldid == "apitype000F1" || thisFldid == "structcapui000F1" || thisFldid == "unameui000F1" || thisFldid =="objdatasrc000F1") {
                            ...
                            $.each(tstDCJSON["result"]["row"], function (i, e) {
                                ...
                                $.each(tempfldJSON["result"]["row"], function (ind, ele) {
                                    if (ele.dcname == dcName) {
                                        if (fldCnt == 1)
                                            _strFldDetails += '"' + ele.fname + '":"FIELD_VALUE"';
                                        else
                                            _strFldDetails += ',"' + ele.fname + '":"FIELD_VALUE"';
                                        fldCnt++;
                                    }
...
function saveFillgridColResize(_thisDcno, _thisFillgridName) {
    ...
    ShowDimmer(true);
    try {
        ASB.WebService.SaveFillgridResize(transid, _thisFillgridName, _strWidth, SuccessCallbackFGResize, OnExceptionFGResize);
    }
    catch (exp) {
        ShowDimmer(false);
    }
