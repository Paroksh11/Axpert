...
$j(document).ready(function () {
    ...
    if (typeof AxpTstButtonStyle != "undefined" && AxpTstButtonStyle == "old") {
        ...
        try {
            ...
            menu.on("kt.menu.dropdown.show", (item) => {
        ...
        if (tablehtml != "" && tablehtml == "<label>No data found.</label>") {
            ...
            if (resSplit.length > 1 && resSplit[1].toString() != "") {
                ...
                let myModal = new BSModal("dvFillGrid", fillTitle, modalData, () => {
                    //shown callback
                    $("#dvFillGrid .dataTables_scrollHeadInner,#dvFillGrid .dataTables_scrollHeadInner .customSetupTableMN").addClass("vw-100");
                    $("#dvFillGrid .dataTables_scrollBody,#dvFillGrid .dataTables_scrollBody .customSetupTableMN").addClass("vw-100");
                    $("#dvFillGrid .dataTables_scroll").addClass("vw-100");

                    //var dddd = $("#dvFillGrid .dataTables_scrollHeadInner").outerWidth();
                    //$("#dvFillGrid .dataTables_scrollBody,#dvFillGrid .dataTables_scrollBody .customSetupTableMN").css({ "width": dddd + "px" });
                    //$("#dvFillGrid .dataTables_scroll").css({ "width": dddd + "px" });
                ...
                myModal.modalBody.classList.add("py-3");
                myModal.modalFooter.classList.add("py-4");
                ...
                var buttonResizeHTML = `<button type="button" class="btn btn-light shadow-sm" onclick="saveFillgridColResize('` + fillGridDc + `','` + AxActivefillGridName + `');">Save Resize</button>`;
...
function DropzoneInit(dvId) {
    ...
    _thisDiv.each(function () {
        ...
        var myDropzone = new Dropzone(id, { // Make the whole body a dropzone
            ...
            clickable: typeof $(id + " .dropzone-select").parents(".grdattch").attr("id") != "undefined" ? "#" + $(id + " .dropzone-select").parents(".grdattch").attr("id") : "#" + $(id + " .dropzone-select").parents(".nongrdattch").attr("id"), // Define the element that should be used as click trigger to select files.
        ...
        myDropzone.on("complete", function (progress) {
            ...
            if (progress.status == 'success') {
                if (progress.xhr.response == "success" || progress.xhr.response.startsWith("success:")) {
                    ...
                    $(progress.previewElement).parents(".dropzone").find("#gridAttachCounter").removeClass("d-none");
...
function DropzoneGridInit(dvId) {
    ...
    _thisDiv.each(function () {
        ...
        myDropzone.on("addedfile", function (file) {
            ...
            _valFiles = "";
            //if ($("#" + funame + "path" + $(id).attr("id").substring($(id).attr("id").lastIndexOf("F") - 3)).length > 0) {
        ...
        ASB.WebService.GetAutoIntelliFromAPI(tstDataId, ChangedFields, ChangedFieldDbRowNo, ChangedFieldValues, DeletedDCRows, _fname, "", "", _resTstHtmlLS, (res) => {
            ...
            try {
                ...
                setTimeout((viewele = matchInfo.autocomplete.view.element.$) => {
                    if (viewele.getBoundingClientRect().y + viewele.getBoundingClientRect().height > document.getElementsByClassName("tstructMainBottomFooter")[0].getBoundingClientRect().y + document.getElementsByClassName("tab-content")[0].getBoundingClientRect().y) {
