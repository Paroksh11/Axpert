...
function AssignJQueryEvents(dcArray) {
    ...
    for (var i = 0; i < dcArray.length; i++) {
        ...
        if ($j(dvId).length > 0) {
            ...
            $j(dvId).find("textarea:not(#txtCommentWF):not(.labelInp,.select2-search__field),[id]:text:not([class=AxAddRows],[class=AxSearchField],.gridRowChk,.tstOnlyTime,.tstOnlyTime24hours),:password").blur(function () {
                MainBlur($j(this));
                if (axInlineGridEdit) {
                    var fldObj = $j(this);
                    var fldId = fldObj.attr("id");
...
function GetFormLoadData(tstQureystr, isDraft, forceRefresh = "false") {
    ...
    try {
        GetProcessTime();
        $.ajax({
            url: '../aspx/tstruct.aspx/GetFormLoadValues',
            type: 'POST',
            cache: false,
            async: true,
            data: JSON.stringify({
